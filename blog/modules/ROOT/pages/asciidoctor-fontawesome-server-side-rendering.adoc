...
In this article, we are going to see how we use this knowledge to render icons as SVG using the _Font Awesome_ JavaScript API.
...
Now that we learned how to use the _Font Awesome_ JavaScript API, let's write a custom converter to use it:
...
<2> Check that the document attribute `icons` is equals to `svg`
<3> Get the icon name using `getTarget()` function
<4> If define, add the `prefix` to the search
<5> If the icon was found, return the SVG representation
<6> If the `node` is not an `icon` or if the document attribute `icons` is not `svg`, delegate to the default HTML5 converter
...
----
<1> `dom.css()` returns a `String` representing all the styles required for displaying icons at the correct size
