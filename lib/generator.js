...
async function generateSite (args, env) {
  ...
  const uiModel = pageComposer.buildSiteUiModel(playbook, contentCatalog)
  const atomFeed = rss.generateAtomFeed(uiModel, articles)
  ...
  siteFiles.push(composePage(createRSSPage(atomFeed), contentCatalog, navigationCatalog))
...
function createRSSPage (feed) {
  return {
    title: `Blog / Feed - Yuzu tech`,
    version: '1.0',
    ...
    out: { path: `blog/1.0/rss/feed.xml` },
    pub: {
      url: `/blog/1.0/rss/feed.xml`,
      moduleRootPath: '.',
      rootPath: '../../../..'
    }
  }
